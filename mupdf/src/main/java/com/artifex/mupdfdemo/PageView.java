...
public abstract class PageView extends ViewGroup {
	...
	public void selectText(float x0, float y0, float x1, float y1) {
		...
		float docRelY0 = (y0 - getTop())/scale;   // todo for selectAll text
		float docRelX1 = (x1 - getLeft())/scale; // x1 - getLeft
		float docRelY1 = (y1 - getTop())/scale; // y1 - getTop
    ...
    public void selectWord(float x0, float y0) {
		float scale = mSourceScale*(float)getWidth()/(float)mSize.x;
		float docRelX0 = (x0 - getLeft())/scale;
		float docRelY0 = (y0 - getTop())/scale;   // todo for selectAll text
//		float docRelX1 = (x0 + 12)/scale; // x1 - getLeft
//		float docRelY1 = (y0 - getTop())/scale; // y1 - getTop
		// Order on Y but maintain the point grouping
//		if (docRelY0 <= docRelY1)
        mSelectBox = new RectF(docRelX0, docRelY0, docRelX0, docRelY0);
